<template>
    <v-layout>
        <v-expansion-panel v-if="result && result.data && result.data.registros && result.data.registros.length">
        ...
        <v-flex xs12 v-else>
            <div class="title text-xs-center grey--text" >La planta no tiene sistemas relacionados.</div>
        </v-flex>
    </v-layout>
</template>
